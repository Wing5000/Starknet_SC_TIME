...
const DEFAULT_RPC_RETRY_BUDGET_MS = 45_000
...
async function callRpcWithRetry<T>(factory: () => Promise<T>, options: RetryOptions): Promise<T> {
  const { method, retryBudgetMs = RPC_RETRY_BUDGET_MS, log } = options
  ...
  for (let attempt = 1; ; attempt += 1) {
    try {
      ...
      if (projectedElapsed > retryBudgetMs) {
        const elapsedSeconds = (elapsedMs / 1000).toFixed(1)
        const budgetSeconds = (retryBudgetMs / 1000).toFixed(1)
        log?.({
          level: 'error',
          message: `[${method}] Retry budget exhausted after ${attempt} attempts (waited ~${elapsedSeconds}s of ${budgetSeconds}s). Consider a longer retry-after or switching RPC endpoint.`
        })
        throw error
      }

      const delaySeconds = delayMs >= 1000 ? `${(delayMs / 1000).toFixed(1)}s` : `${delayMs}ms`
      const remainingBudgetMs = Math.max(0, retryBudgetMs - elapsedMs)
      const remainingLabel = remainingBudgetMs >= 1000
        ? `${(remainingBudgetMs / 1000).toFixed(1)}s`
        : `${remainingBudgetMs}ms`
      log?.({
        level: 'warn',
        message: `[${method}] Rate limited (attempt ${attempt}). Retrying in ${delaySeconds}. Remaining budget ~${remainingLabel}.`
      })
      await sleep(delayMs)
    }
  }
...
export const RPC_RETRY_BUDGET_MS = parsePositiveNumber(
  env?.VITE_RPC_RETRY_BUDGET_MS,
  DEFAULT_RPC_RETRY_BUDGET_MS,
  1000
)
